/* ===========================================================================
   ...
   El plan se calcula sobre el HISTORICO COMPLETO, como las cohortes y las
   recomendaciones: las unidades de inventario son globales por referencia, no
   estan repartidas por tienda, asi que filtrar por pais daria una ocupacion que
   es cota inferior y decisiones que cambiarian al marcar una casilla.
   =========================================================================== */
...
/* Resumen por accion en el modo pedido. */
function fleetSummary(mode) {
  const P = fleetPlan();
  return FLEET_ACTIONS.map(a => {
    const set = P.rows.filter(d => fleetPick(d, mode).action === a.id);
    return {
...
function pageFlota(grid) {
  ...
  /* -- 1. El plan --------------------------------------------------------- */
  const c1 = card("c12", "El plan", "¿Qué toca hacer con cada referencia, y cuánto mueve?",
    "histórico completo");
  grid.appendChild(c1);
  segmented(c1, [{
  ...
  const drawPlan = () => {
    const sum = fleetSummary(fleetView.mode);
  ...
  const c2 = card("c6", "Qué cambia al imputar el material",
    "¿Cuántas decisiones dependen de un coste que hoy no se descuenta?", "histórico completo");
  grid.appendChild(c2);
  const before = fleetSummary("contrib"), after = fleetSummary("neto");
  const nodes = [], links = [];
  ...
  Object.entries(P.flow).forEach(([k, v]) => {
  ...
  setTable(c2, ["Sin imputar", "Imputado", "Referencias"],
    Object.entries(P.flow).sort((a, b) => b[1] - a[1]).map(([k, v]) => {
      const [from, to] = k.split("|");
      return [fleetLabel(from), fleetLabel(to),
              nf(v) + (from === to ? "" : ' <span class="sig">cambia</span>')];
    }));
  const movedCapital = P.rows.filter(d => d.planContrib.action !== d.planNeto.action)
    .reduce((a, d) => a + d.capital, 0);
  const keptButUnpaid = P.rows.filter(d => d.planContrib.action === "mantener" &&
                                           d.planNeto.action !== "mantener").length;
  insight(c2, "<b>" + nf(P.changed) + " de " + nf(P.rows.length) + " decisiones cambian</b> (" +
    pct(P.changed / P.rows.length, 0) + " del catálogo, " + e(movedCapital) + " de capital) sólo " +
    "por descontar lo que cuesta el material. Sin imputarlo el plan manda <b>mantener</b> " +
    nf(before.find(d => d.id === "mantener").refs) + " referencias, y <b>" + nf(keptButUnpaid) +
  ...
  const c3 = card("c6", "La regla, de un vistazo",
    "¿Por qué le toca a cada referencia lo que le toca?", "histórico completo");
  grid.appendChild(c3);
  ...
  const drawRule = () => {
    ...
    lg.className = "legend";
    lg.innerHTML = FLEET_ACTIONS.filter(a => P.rows.some(d => fleetPick(d, m).action === a.id))
  ...
  const c4 = card("c12", "Las " + nf(P.rows.length) + " referencias",
    "Ordenadas por el euro anual que mueve su acción.", "histórico completo");
  grid.appendChild(c4);
  tableOnly(c4, ["Producto", "Categoría", "Acción", "Por qué", "Uds.", "Ocupación",
                 "Contrib./ud·año", "Amort./ud·año", "Neto/ud·año", "Payback",
                 "€/año en juego", "Capital"], []);
  const drawDetail = () => {
    const m = fleetView.mode;
    setTable(c4, ["Producto", "Categoría", "Acción", "Por qué", "Uds.", "Ocupación",
                  "Contrib./ud·año", "Amort./ud·año", "Neto/ud·año", "Payback",
                  "€/año en juego", "Capital"],
      P.rows.slice()
        .sort((a, b) => (fleetPick(b, m).gain || 0) - (fleetPick(a, m).gain || 0))
        .map(d => [d.name, D.categories[d.cat],
          '<b style="color:' + fleetColor(fleetPick(d, m).action) + '">' +
            fleetLabel(fleetPick(d, m).action) + "</b>",
          fleetPick(d, m).why, nf(d.units), pct(d.occ, 2),
          eur(d.contribU, 2), eur(d.amortU, 2), eur(d.netU, 2),
          isFinite(d.payback) ? nf(d.payback, 1) + " a" : "nunca",
          e(fleetPick(d, m).gain), e(d.capital)]));
  };
